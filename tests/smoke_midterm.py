...
def main() -> None:
    ...
    dataset = assert_ok(client.post("/api/datasets/load", json={}), "load dataset")
    ...
    index = assert_ok(client.post("/api/index/build", json={}), "build index")
    print("Index:", index["index_type"], index["mode"], index["vector_count"], "vectors")

    sample_cell_id = dataset["sample_cell_ids"][0]
    result = assert_ok(client.post("/api/search", json={"cell_id": sample_cell_id, "top_k": 5}), "search")
